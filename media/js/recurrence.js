...
function output_recurrencescript() {
	...
	if ($select_value != 0) {	// want the user a recurrence
		...
		$("recurrence_interval").value = 0;	// set the parameter
...
function generate_selectlist($select_value) {
	...
	for ($j = 0; $j < $limit; $j++) {
		...
		if ($j == (parseInt($("recurrence_interval").value) - 1)) {	// the selected - attribute
...
function set_parameter() {
	// include the value into the recurrence_interval input tag
	$("recurrence_interval").value = $("recurrence_selectlist").value;
}